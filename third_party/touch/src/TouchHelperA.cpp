...
// 包含 Nuklear 头文件以使用 nk_context 和相关函数
#include <nuklear.h>
#include "TouchHelperA.h"
#include "Utils.h"
// 必须添加的3个头文件
#include <dirent.h>   // 解决 DIR、dirent、opendir 相关错误
#include <fcntl.h>    // 解决 O_RDWR、O_NONBLOCK 相关错误
#include <linux/input.h>  // 解决 input_event、EV_KEY、KEY_VOLUMEUP/DOWN 相关错误
...
namespace Touch {
    ...
    // --- 用于存储Nuklear输入状态的静态变量 ---
    static My_Vector2 nuklear_touch_pos = {-1.0f, -1.0f};
    static bool nuklear_touch_down = false;
    // --- 结束部分 ---
        ...
        while (initialized) {
            ...
            for (size_t j = 0; j < count; j++) {
                ...
                if (ie.code == SYN_REPORT) {
                    ...
                    for (int finger_idx = 0; finger_idx < 10; ++finger_idx) {
                        if (device.Finger[finger_idx].isDown) {
                            nuklear_touch_pos = Touch2Screen(device.Finger[finger_idx].pos);
                            any_finger_down = true;
                            break; 
                        }
                    }
                    nuklear_touch_down = any_finger_down;
                    
                    if (!any_finger_down) {
                        nuklear_touch_pos = {-1.0f, -1.0f};
                    }
    ...
    void Close() {
        if (initialized) {
            initialized = false; // 先设置标志位，让线程退出
            sleep(1); // 等待线程响应
            for (auto &device: devices) {
                if (!readOnly)
    ...
    void UpdateNuklearInput(nk_context* ctx) {
        ...
        static My_Vector2 last_valid_pos = {-1.0f, -1.0f};

        lock.lock();
        My_Vector2 current_pos = nuklear_touch_pos;
        bool current_down = nuklear_touch_down;
        lock.unlock();
        ...
        // 2. 始终在最后一个有效坐标处报告运动
        // 这可以确保控件即使在手指抬起后也能正确地失去悬停状态
        if (last_valid_pos.x >= 0 && last_valid_pos.y >= 0) {
            nk_input_motion(ctx, (int)last_valid_pos.x, (int)last_valid_pos.y);
        }

        // 3. 仅当按钮状态改变时才发送事件
        if (prev_down != current_down) {
            // [关键] 使用最后一个有效坐标来报告按钮事件，以确保拖动正确
            nk_input_button(ctx, NK_BUTTON_LEFT, (int)last_valid_pos.x, (int)last_valid_pos.y, current_down);
